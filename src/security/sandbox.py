...
class SecuritySandbox:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.file_read_whitelist: Set[str] = set(
            self.config.get("file_read_whitelist", [])
        )
        self.file_write_whitelist: Set[str] = set(
            self.config.get("file_write_whitelist", [])
        )
    ...
    def check_resource_limits(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            self._audit_log(
                "resource_check",
                "SUCCESS",
                {"plugin": plugin_name, "memory_mb": memory_mb},
            )
            return True
    ...
    def restrict_file_access(
        self, read_paths: List[str], write_paths: List[str]
    ) -> None:
            ...
            if perm in dangerous_permissions:
                ...
                if not plugin_meta.get("security", {}).get("approved", False):
                    self._audit_log(
                        "permission_check",
                        "DENIED",
                        {"permission": perm, "reason": "Requires explicit approval"},
                    )
                    return False
        ...
        for host in network_whitelist:
            if not self.is_host_allowed(host):
                self._audit_log(
                    "network_check",
                    "DENIED",
                    {"host": host, "reason": "Not in whitelist"},
                )
                return False
        ...
        for path in file_read_paths:
            if not self.is_path_readable(path):
                self._audit_log(
                    "file_check",
                    "DENIED",
                    {"path": path, "reason": "Not in read whitelist"},
                )
                return False
