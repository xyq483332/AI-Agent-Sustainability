...
from src.api.main import app, plugin_manager, security_sandbox, metrics_collector
...
class TestPluginEndpointsIntegration:
    ...
    def test_register_plugin_success(self, client, mock_plugin_manager, mock_security_sandbox):
    ...
    def test_register_plugin_security_failure(self, client, mock_plugin_manager, mock_security_sandbox):
    ...
    def test_register_plugin_load_failure(self, client, mock_plugin_manager, mock_security_sandbox):
...
class TestSecurityEndpointsIntegration:
    ...
    def test_get_audit_log(self, client, mock_security_sandbox):
        ...
        mock_audit_log = [
            {"action": "test", "status": "SUCCESS", "timestamp": datetime.utcnow().isoformat()}
        ]
...
class TestAPIIntegrationLifecycle:
    ...
    def test_plugin_lifecycle_api(self, client, mock_plugin_manager, mock_security_sandbox):
        ...
        response = client.post("/api/v1/plugins/test_plugin/execute", json={"context": {}})
        assert response.status_code == 200
